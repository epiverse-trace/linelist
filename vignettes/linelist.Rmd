...
A `linelist` object is an instance of a `safeframe`, which is a `data.frame` or
a `tibble` in which key variables have been *labelled*. The main features of the
package is broken down into the 3 categories outlined above. In linelist, 
epidemiological data is labelled by default.

### Labelling system

Labels are descriptions of epidemiological variables attached to the column 
(that is, variable) in a `data.frame` or `tibble`. We use R's built in label
attribute to be interoperable with other packages. 

* `make_linelist()`: to create a `linelist` object by labelling key epi 
  variables in a `data.frame` or a `tibble` using a set of defaults.

* `set_labels()`: to add, remove, or modify tags in a `linelist`

* `labels()`: to list variables which have been labelled in a `linelist`

* `label_defaults()`: to list all default variable names and labels; details on
   what the tags represent can be found at [`?make_linelist`](https://epiverse-trace.github.io/linelist/reference/make_linelist.html)

* `labels_df()`: to obtain a `data.frame` of all the labelled variables in a `linelist`
...
* `validate_labels()`: check that labelled variables are present in the dataset,
  that labels match the pre-defined list of labelled variables

* `validate_types()`: check that labelled variables have an acceptable class,
  as defined in `vars_types()`

* `validate_linelist()`: general validation of *linelist* objects, equivalent to
  running both `validate_labels()` and `validate_types()`, and checking the
  class of the object

### Secured methods

These are dedicated S3 methods for existing generics which can be used to
prevent the loss of tagged variables. These are implemented in `safeframe` but
where relevant, they are re-exported in `linelist`.

* `lost_labels_action()`: to set the behaviour to adopt when labelled variables
  would be lost by an operation: issue a warning (default), an error, or ignore

* `get_lost_labels_action()`: to check the current behaviour for lost, labelled
  variables
...
* `x[]` and `x[[]]`: for subsetting columns using 'base R' syntax; will
  behave according to `get_lost_labels_actions()` if tagged variables are lost
...
Let us assume we want to label the following variables to facilitate downstream
...
All the default variables named consistently in your dataset, will automatically
be labelled as well. In the example, `age` and `gender` match exactly and are
auto-labelled. See `default_vars()` for the full list of default names.

The printing of the object confirms that the labels have been added. If we want to
...
Labels can be added / removed / changed using `set_labels()`.
...
x <- x %>%
  ...
  set_labels(inferred_outcome = "Inferred outcome of disease")
x
```

If we wanted to undo the above, i.e. remove the `outcome` label, we only need to set
it to `NULL`:

```{r}
x <- x %>%
  set_labels(inferred_outcome = NULL)
labels(x)
```


## Accessing labelled variables

Now that key variables have been tagged in `x`, we can use these pre-defined
fields in downstream analyses, without having to worry about variable names and
types. We could access labelled variables using any of the following means:

```{r}
# select tagged variables only
x %>%
  select(has_label(c("date_of_prodrome", "date_of_death")))

# select tagged variables only with renaming on the fly
x %>%
  select(onset = has_label("date_onset"))
...
some of the variables that were labelled.

We can also use the `has_label()` select helper to select columns via their 
label. For example, to retain the first 2 variables, and the `gender` label:

```{r}
# hybrid selection
x %>%
  select(1:2, has_label("gender"))
```

Again, we observe a warning as before due to the loss of labelled variables in
the operation. This behaviour can be silenced if needed, or could be changed to
...
# note that `lost_labels_action` sets the behavior for any later operation, so
# we need to reset the default
get_lost_labels_action() # check current behaviour
lost_labels_action() # reset default
```

### Changing tag loss action permanently

If you wish to change the `lost_labels_action` in a way that persists across R sessions, you can do so by setting the `SAFEFRAME_LOST_ACTION` environment variable. For example, your `.Renviron` file could contain the following line:

```text
SAFEFRAME_LOST_ACTION="error"
```

It is originally implemented in the `safeframe` package, which is why it is not called `LINELIST_LOST_ACTION`.
