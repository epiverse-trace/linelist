...
* `labels()`: to list variables which have been tagged in a `linelist`

* `labels()`: to list all recognized tag names; details on what the tags represent can be found at [`?make_linelist`](https://epiverse-trace.github.io/linelist/reference/make_linelist.html)
...
* `validate_labels()`: check that tagged variables are present in the dataset,
...
* `validate_linelist()`: general validation of *linelist* objects, equivalent to
  running both `validate_labels()` and `validate_types()`, and checking the class
  of the object
...
* `lost_labels_action()`: to set the behaviour to adopt when tagged variables
  would be lost by an operation: issue a warning (default), an error, or ignore

* `get_lost_labels_action()`: to check the current behaviour for lost tagged
  variables
...
We can also use the `has_label()` select helper to select columns via their tag.
...
If you wish to change the `lost_labels_action` in a way that persists across R sessions, you can do so by setting the `LINELIST_LOST_ACTION` environment variable. For example, your `.Renviron` file could contain the following line:
